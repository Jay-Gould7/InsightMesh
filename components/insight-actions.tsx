...
export function InsightActions({ bountyId }: { bountyId: number }) {
  ...
  async function analyze() {
    setError("");
    setIsAnalyzing(true);
    const response = await fetch("/api/ai/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bountyId }) });
  ...
  async function freezeSnapshot() {
    setError("");
    setIsFreezing(true);
    const response = await fetch("/api/ai/score", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bountyId }) });
    const payload = await response.json();
    setIsFreezing(false);
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button onClick={analyze} disabled={isAnalyzing} className="rounded-full bg-cyan-300 px-4 py-2 text-sm font-semibold text-stone-900 disabled:opacity-60">{isAnalyzing ? "Analyzing..." : "Preview analysis"}</button>
        <button onClick={freezeSnapshot} disabled={isFreezing} className="rounded-full border border-white/15 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{isFreezing ? "Freezing..." : "Freeze score snapshot"}</button>
      </div>
